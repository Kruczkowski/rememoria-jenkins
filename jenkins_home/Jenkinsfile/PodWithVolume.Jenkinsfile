...
pipeline {
    agent any
    parameters {
        string(name: 'GPU_TYPE', defaultValue: 'NVIDIA RTX 2000 Ada Generation', description: 'Typ GPU')
        string(name: 'DISK_SIZE', defaultValue: '10', description: 'Rozmiar dysku w GB')
        string(name: 'NETWORK_VOLUME_ID', defaultValue: '', description: 'ID Network Volume do podmontowania')
        string(name: 'IMAGE', defaultValue: 'ghcr.io/kruczkowski/rememotion-python-base:latest', description: 'Obraz Docker')
    }
    stages {
        stage('Create RunPod Pod with Network Volume') {
            steps {
                withCredentials([
                    ...
                    script {
                        ...
                        writeFile file: 'payload.json', text: groovy.json.JsonOutput.toJson([
                            ...
                            networkVolumeId          : params.NETWORK_VOLUME_ID,
